...
def rock_superstars
  ...
  execute(<<-SQL)
    ...
    JOIN
      albums ON rock_albums.album = albums.asin
    GROUP BY
      albums.artist
    ORDER BY
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      style, SUM ( COALESCE (album_data.price, 0)) / SUM (album_data.album_length) AS average_price
    FROM
      styles
    JOIN(
      SELECT
        asin, title, price, COUNT (tracks.song) AS album_length
      FROM
        albums
      JOIN
        tracks ON albums.asin = tracks.album
    ...
    ON styles.album = album_data.asin
    GROUP BY
      styles.style
    ORDER BY
      average_price DESC
